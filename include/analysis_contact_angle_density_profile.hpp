...
#include <Eigen/Dense>
#include <Eigen/QR>
#ifndef ANALYSIS_CONTACT_ANGLE_DENSITY_PROFILE_HPP
#define	ANALYSIS_CONTACT_ANGLE_DENSITY_PROFILE_HPP

using namespace std;

class  ANALYSIS_CONTACT_ANGLE_DENSITY_PROFILE : public ANALYSIS
{
...
//    GROUP *sel1;
 //   int whichN;
    ...
    ofstream *density_file_solute = new ofstream("density_2D_solute.dat");

    vector<vector<float>> density_yz;
    vector<vector<float>> density_yz_solute;
    string contact_angle_filename;
    vector<float> density_z_solute;
    string fitting_function;
    vector<double> coeff;

    ANALYSIS_CONTACT_ANGLE_DENSITY_PROFILE(PSF *system, GROUP *sel1, GROUP *sel2, int vector1d, int vector2d, int voidf, string filename, string contact_angle_filename, float zshift, float dr, float zlower, string fitting_function, int every_n_frame); //constructor
    ...
    void output_density_solute(vector<vector<float>> density_yz_solute,  vector<double> coeff);
    ...
    ~ANALYSIS_CONTACT_ANGLE_DENSITY_PROFILE();
