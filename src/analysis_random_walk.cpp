...
ANALYSIS_RANDOM_WALK::ANALYSIS_RANDOM_WALK(PSF *system, GROUP *sel1, int vector1d, int vector2d, int voidf, float bl, string filename, int wall_rw, vector<PSF*> monomers, float fringe) //constructor
{
    this->system = system;
    this->sel1 = sel1;
...
vector<vector<float>> ANALYSIS_RANDOM_WALK::bond_next(int residueid, PSF *monomer) {
    ...
    for (int ind : monomer->residues[residueid]) {
      ...
      //       cout << "testing C1-4: " << ind_a1 << " " << ind_a2 << endl;
...
//	 }
    ...
    vector<float> vec1 = {monomer->x[ind_a2_ref],monomer->y[ind_a2_ref],monomer->z[ind_a2_ref]};
//    vector<float> vec2 = {monomer->x[ind_ref_H],monomer->y[ind_ref_H],monomer->z[ind_ref_H]};
...
void ANALYSIS_RANDOM_WALK::compute_void() {
    FILE * fpdb;
    const char *filechar = this->filename.c_str();
...
//Determine if a residue in the system psf have different stereochemistry (cis or trans. If yes, len(ctids)>1)
	...
	ctids[residuename] = ctid;
        cout << "ctids: " << residuename << " " << ctid.size() << endl;
    }
...
//grow the polymer monomer by monomer by ramdom walk
//
...
// determine the cis or trans reference monomer
...
// determine the reference atoms for monomer[restempid]
	    ...
	    if (monomers[moid]->monomerflag < 3) {
		 ...
		 for (int ib = 0; ib < system->nbonds_atom[ind]; ib++) {
		     int ind_temp = system->ibond_atom[ind][ib];
                     if (system->atomname[ind_temp] == monomers[moid]->name_ref2 && system->resid[ind_temp] == system->resid[ind]) {
   	                 ind_a3 = ind_temp; // determine the atom in the next monomer that is connected to this monomer
	             } else if (system->atomname[ind_temp] == monomers[moid]->name_ref1 && system->resid[ind_temp] == system->resid[ind]) {
	               ind_H = ind_temp;
		     }
...
// Assigning atomic positions of each residue by its corresponding monomer
	int atid = 1;
//	if (monomers[moid]->ind_a1 > monomers[moid]->ind_a2 && restempid != sel1->residues.size()-1) {
        if ( monomers[moid]->monomerflag == 1 ) {
	    atid = 0;
	}
	for (int ind: sel1->residues[restempid]) {
...
//step1: attach the bond connecting this and the next monomer and the relaxed orientation based on the other atoms in the current monomer connected to C4 
 	//    if (restempid == 1) {cout << "a4_pre: " << ind_a2_pre << " ind_H_pre: " << ind_H_pre << " a3_pre: " << ind_a3_pre << endl;}	  
//MSe comment: bond rotation

// This step is to determine the direction of the next bond. Since the next atom is not included in the system psf, this tentative rotation is necessary.
//
	    vec_CC = bond_rotate_vec(ind_a2_pre, ind_H_pre, ind_a3_pre, monomers[moid_pre]->Vch, monomers[moid_pre]->Vcc, monomers[moid_pre]->Vcc_next); //Note the coordinates are updated in the chain, but the vectors are original monomers with connected atom at each end.


            //vec_CC[0] = system->x[ind_a3_pre] - system->x[ind_a2_pre];
            //vec_CC[1] = system->y[ind_a3_pre] - system->y[ind_a2_pre];
            //vec_CC[2] = system->z[ind_a3_pre] - system->z[ind_a2_pre];

            shift[0] = (system->x[ind_a2_pre] + vec_CC[0]) - system->x[ind_a1];
            shift[1] = (system->y[ind_a2_pre] + vec_CC[1]) - system->y[ind_a1];
            shift[2] = (system->z[ind_a2_pre] + vec_CC[2]) - system->z[ind_a1];
...
//            cout << "output theta: " << theta_random << " " << cos(theta_random) << endl;// debug
            ...
            residue_rotate_random(ind_a3_pre, ind_a2_pre, ind_a2_pre, theta_random, restempid, ind_a2_pre, ind_a2);


            if (this->wall_rw == 1) {
	        if (hitwall[0] == 0 && hitwall[1] == 0 && hitwall[2] == 0 && hitwall[3] == 0 && hitwall[4] ==0 && hitwall[5] == 0) {
   	            update_res(restempid, ind_a2_pre);
  	        } else {
...
/////////step 3: rotate the monomer along the C4_pre-C1 bond
	    if ( ind_a1 != ind_a2 ) {
                ...
                fill(hitwall.begin(), hitwall.end(),0);
                residue_rotate_random_regular(ind_a2_pre, ind_a1, ind_a1, theta_random, restempid, ind_a2);


                if (this->wall_rw == 0) {

	    	    update_res_regular(restempid);
                } else if (hitwall[0] == 0 && hitwall[1] == 0 && hitwall[2] == 0 && hitwall[3] == 0 && hitwall[4] ==0 && hitwall[5] == 0) {
	    	    update_res_regular(restempid);
		} else {
		    ...
		    float E_res_pre = E_res;
  	            float r2_a2_pre = r2_a2;
// Following: Choose the move leading to a residue as close as possible to the center.
		    for (int mcstep = 0; mcstep < 50; mcstep++) {
	                theta_random = 2 * PI * float(mcstep) * 0.02;
                	residue_rotate_random_regular(ind_a2_pre, ind_a1, ind_a1, theta_random, restempid, ind_a2);
		   //     if (E_res < E_res_pre) {
		        if (r2_a2 < r2_a2_pre) {
		//	    E_res_pre = E_res;
...
///////////////end rotation part////////////////////
    ...
    for (int restempid = 0; restempid < sel1->residues.size(); restempid++) {
        for (int ind : sel1->residues[restempid]) {
            fprintf(fpdb,"%4s%7d  %-4s%-4s%1d%4d%12.3f%8.3f%8.3f%6.2f%6.2f\n","ATOM", ind + 1, system->atomname[ind].c_str(), system->resname[ind].c_str(), 1, system->resid[ind], system->x[ind], system->y[ind], system->z[ind], 0.0,0.0);
        }
    }
